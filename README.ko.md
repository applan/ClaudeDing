...
- ⏰ **리마인드** — **권한 승인 대기**(작업 도중 막힘)를 응답 없이 두면 설정 주기(기본 **1분**)마다 다시 알려줍니다. 응답하면 자동으로 멈춰요. 모든 작업이 끝난 뒤의 **입력 대기**는 한 번만 울립니다.
...
**권한 승인 대기**(작업 도중 막힌 상태) 알림을 응답 없이 두면 일정 주기로 다시 알려줍니다. 모든 작업이 끝난 뒤의 **입력 대기** 상태는 한 번만 울리고 끝냅니다. `~/.claude/claudeding.config.json`에서 조정하세요(없으면 기본값으로 동작):
